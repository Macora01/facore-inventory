...
async function startServer() {
  ...
  // Con productId: borra duplicados de ese producto. Sin productId: TODOS los productos.
  app.post('/api/emergency/dedup-init-load', asyncHandler(async (req: Request, res: Response) => {
    ...
    if (productId) {
      ...
      if (dups.rows.length <= 1) {
        return ok(res, { message: `Producto ${productId} tiene ${dups.rows.length} INITIAL_LOAD(s). Nada que deduplicar.`, productsAffected: 0 });
      }
      const [, ...remove] = dups.rows;
      ...
      return ok(res, {
        productsAffected: 1,
        removedMovements: delResult.rowCount,
        message: `Producto ${productId}: ${delResult.rowCount} INITIAL_LOAD eliminados`
      });
    }

    // Modo masivo: todos los productos con INITIAL_LOAD duplicados
    // Encontrar IDs duplicados (conservando el más antiguo por producto)
    const dups = await p.query(`
      SELECT id, product_id, quantity, timestamp FROM movements
      WHERE type = 'INITIAL_LOAD'
        AND id NOT IN (
    ...
    if (dups.rows.length === 0) {
      return ok(res, { message: 'No hay INITIAL_LOAD duplicados.', productsAffected: 0, removedMovements: 0 });
    }

    const idsToDelete = dups.rows.map((r: any) => r.id);
    const productsSet = new Set(dups.rows.map((r: any) => r.product_id));
    const totalQty = dups.rows.reduce((sum: number, r: any) => sum + Number(r.quantity), 0);
    ...
    ok(res, {
      ...
      removedMovements: delResult.rowCount,
      totalDuplicatedQty: totalQty,
      message: `Deduplicados ${productsSet.size} productos: ${delResult.rowCount} INITIAL_LOAD eliminados (${totalQty} unds duplicadas)`
    });
  }));
